...
async def get_auth_context(
    ...
    # Legacy transport fallback (disabled by default).
    allow_legacy_transport = (
        os.getenv("ALLOW_LEGACY_TOKEN_TRANSPORT", "").strip().lower()
        in {"1", "true", "yes"}
    )
    ...
    # Test compatibility path for existing fixtures that use non-JWT test tokens.
    ...
    # Fast reject malformed bearer values before calling auth server.
    if token.count(".") != 2:
        raise HTTPException(
    ...
    try:
        ...
        allow_local_fallback = (
            os.getenv("ALLOW_LOCAL_JWT_FALLBACK", "").strip().lower()
            in {"1", "true", "yes"}
        )
